...
void help()
{
    string str =
	"\n"
    "MatchScan: a program to find significant matches between a query sequence and a set of target sequences, where the frequency of the matched sequences in targets correlates with target scores\n"
	"	- Xuebing Wu (wuxb07@gmail.com)\n"
    "\n"
    "Usage: MatchScan -q query.fasta -t target.fasta -s score.tab -o output [ -k 6,10 -h highlight.fasta]\n"
    "\n"
    "Options:\n"
    "\n"
    "   -q  file   query sequence file, fasta\n"
    "   -t  file   target sequence file, fasta\n"
    "   -s  file   score file (ID in column 1, score in column 2)\n"
	"   -o  file   output file (tabular data and PDF figure)\n"
	"   -h  file   query sequences to highlight in green, such as ChIRP probes\n"
	"   -k  num    kmer length: -k 6, or range: -k 6,10, i.e. from 6 to 10\n"		
