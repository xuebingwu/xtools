...
map<string,double> load_weight(string filename, int c1, int c2, bool skip_line1/*=false*/)
{
	c1 = c1 - 1;
	c2 = c2 - 1;
    ...
    if(skip_line1) getline(fin,line);
	...
	while(fin)
	{
		...
		flds = string_split(line,"\t");
        //cout << flds[c2] << endl;
        if (flds[c2] == "NA") flds[c2] = "0";
        else if (flds[c2] == "Inf") flds[c2] = "1000";
        else if (flds[c2] == "-Inf") flds[c2] = "-1000";
...
// from a fasta file, select a subet with headers specified by another file 
...
int select_multi_lines(string inputfile, string idfile, string outputfile, int nline/*=2*/, int id_col/*=1*/, string id_prefix/*=">"*/)
{
	...
	while(fin)
	{
        ...
        if(flds.size() <= id_col)
        {
            cout << "Not enough columns for ID list file: "+idfile << endl;
            exit(1);
        }
		ids.insert(id_prefix+flds[id_col]);
	}
	fin.close();
    message(to_string(ids.size())+" IDs loaded");
	...
	while(fin)
	{
		...
		if(line.substr(0,id_prefix.size()) == id_prefix)
		{
			if (ids.find(line) != ids.end())
			{
				selected++;
				out << line << endl; // id
                // print fixed number of lines after ID line
...
void mergeTab(string file1, string file2, string outputfile, unsigned col1/*=0*/, unsigned col2/*=0*/, bool header/*=false*/, string fill/*="None"*/)
{
	...
	*/
    ...
    // get number of columns of input files

    ifstream f1,f2;
    vector<string> flds;
	string line;
    unsigned nc1,nc2; // number of columns

	f1.open(file1.c_str());
    getline(f1,line);
    flds = string_split(line,"\t");
    nc1 = flds.size();
    message(to_string(nc1) + " columns in " +file1);
    f1.close();
    ...
    flds = string_split(line,"\t");
    nc2 = flds.size();
    message(to_string(nc2) + " columns in " +file2);
    f2.close();

    // load file 2 data first
    f2.open(file2.c_str());
	...
	if (fill != "none") 
	{
		for (int i=1;i<nc2;i++) fillline += "\t" + fill;
	}
    ...
    // delete the key column in file 2
    string tmp = random_string(10);
    string cmd = "cut -f -"+to_string(nc1+col2)+","+to_string(nc1+col2+2)+"- "+outputfile+" > " + tmp + "; mv "+tmp+" "+outputfile;
    system(cmd.c_str());
