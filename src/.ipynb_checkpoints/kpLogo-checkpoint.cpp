	...
	if(analysis != "default")
	{
		...
		else // weighted sequecnes 
		{
			...
			if(degenerate) nSig = find_significant_kmer_from_weighted_sequences(
				seqs1, weights, dkmers, outtmp, nTest, pCutoff, Bonferroni, min_shift, max_shift, startPos,minCount,alphabet);
			else 
			{
				message("Performing weighted exact kmer analysis...");
				nSig = find_significant_kmer_from_weighted_sequences(
				seqs1, weights, kmers, outtmp, nTest, pCutoff, Bonferroni, min_shift, max_shift, startPos,minCount,alphabet);
			}
		}
