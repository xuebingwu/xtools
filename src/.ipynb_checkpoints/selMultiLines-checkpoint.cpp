...
void help()
{
    ...
    "Options:\n"
    "\n"
    "   -i  file        Input file such as fasta/fastq. ID line followed by a fixed number of lines per record\n"
	"   -c  column      IDs in this column in the ID file, 1-based\n"
	"   -id_file file   file containing IDs to be selected\n"
    "   -o  file        output file\n"
	"   -n  num         each record contains n lines (e.g. 2 for fasta, 4 for fastq)\n"
	"   -id_prefix ch   prefix for ids in input file but not in id_file, such as > for fasta\n"
    "\n";
